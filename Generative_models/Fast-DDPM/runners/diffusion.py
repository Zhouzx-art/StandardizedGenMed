...
from datasets.ISIC import ISIC
...
class Diffusion(object):
    ...
    def sg_train(self):
        ...
        elif self.args.dataset=='ISIC' or self.args.dataset=='Nuclei' or self.args.dataset=='Polyp' or self.args.dataset=='Fundus' or self.args.dataset=='Prostate' or self.args.dataset=='FeTS' or self.args.dataset=='Chaos':
            dataset = ISIC(self.config.data.train_dataroot, self.config.data.image_size, split='train')
            print('Start training your Fast-DDPM model on '+self.args.dataset+' dataset.')
        ...
        for epoch in range(start_epoch, self.config.training.n_epochs):
            ...
            if epoch % 1 == 0:
                ...
                path=os.path.join(self.args.log_path, "time_log.txt")
                with open(path, 'a') as f:
                    f.write(f'Step {epoch}: {elapsed_time:.2f} seconds\n')
                print(f"Time at epoch {epoch}: {elapsed_time:.2f} seconds")
    ...
    def sg_ddpm_train(self):
        ...
        if self.args.dataset=='LDFDCT':
            ...
            dataset = ISIC(self.config.data.train_dataroot, self.config.data.image_size, split='train')
            print('Start training DDPM model on '+self.args.dataset+' dataset.')
    ...
    def sg_sample_fid(self, model):
        ...
        if self.args.dataset=='LDFDCT':
            ...
            # BRATS for brain image translation
            sample_dataset = ISIC(self.config.data.sample_dataroot, self.config.data.image_size, split='train')
            print('Start training model on  your dataset.')
        print('The inference sample type is {}. The scheduler sampling type is {}. The number of involved time steps is {} out of 1000.'.format(self.args.sample_type, self.args.scheduler_type, self.args.timesteps))
        ...
        with torch.no_grad():
            ...
            for batch_idx, sample in tqdm.tqdm(enumerate(sample_loader), desc="Generating image samples for FID evaluation."):
                ...
                '''
                for i in range(n):
                    name = case_name[i]   # 每一张图对应自己的 case_name
                    tvu.save_image(
                        x_tensor[i],
                        os.path.join(
