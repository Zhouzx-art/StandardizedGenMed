...
class ConditionFuser(nn.Module):
    def __init__(self, dim, mode="concat"):
        super().__init__()
        self.mode = mode
        if mode == "concat":
            self.fuse = nn.Sequential(
                nn.Linear(dim * 2, dim),  # 拼接后的维度是 2*dim
                nn.SiLU(),
                nn.Linear(dim, dim),
            )
        elif mode == "add":
            self.fuse = nn.Identity()  # 直接相加，无需额外处理
        else:
            raise ValueError("Unsupported fusion mode. Use 'concat' or 'add'.")

    def forward(self, t_emb, m_emb):
        if self.mode == "concat":
            # 拼接：t + mask（B, D）
            x = torch.cat([t_emb, m_emb], dim=-1)  # (B, 2*D)
            return self.fuse(x)  # 输出 (B, D)
        elif self.mode == "add":
            # 相加：t + mask（B, D）
            return t_emb + m_emb
...
class DiT_Llama(nn.Module):
    def __init__(self, in_channels=3, input_size=256, patch_size=2, dim=512,
                 n_layers=5, n_heads=16, multiple_of=256, ffn_dim_multiplier=None,
                 norm_eps=1e-5, class_dropout_prob=0.1):
        super().__init__()
        ...
        self.condition_fuser = ConditionFuser(dim, mode="add")  # 使用concat或add
        # 实际推荐设置
        

        self.layers = nn.ModuleList([
            TransformerBlock(layer_id, dim, n_heads, multiple_of,
                             ffn_dim_multiplier, norm_eps)
    ...
    def forward(self, x, t, mask):
        ...
        m_emb = mask      # (B, D)
        m_emb = m_emb.view(m_emb.shape[0], -1)  # 展平成 [8, 1024]
        # 融合 t 和 mask 条件
        #adaln_input = self.condition_fuser(t_emb.to(x.dtype),
        #                               m_emb.to(x.dtype))
        adaln_input = t_emb.to(x.dtype) + m_emb.to(x.dtype)#可以不用相加，可以采用拼接的形式
