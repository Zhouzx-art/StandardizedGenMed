...
class TrainingConfig:
    ...
    learning_rate: float = 1e-4     #灰度图：1e-4,Origa:5e-5
    lr_warmup_steps: int = 10000     #灰度图:500,Origa:10000
...
def train_loop(config, model, noise_scheduler, optimizer, train_dataloader, eval_dataloader, lr_scheduler, device='cuda',early_stop=1):
    # Prepare everything
    ...
    original_eval_dataloader = eval_dataloader
    ...
    for epoch in range(start_epoch, config.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            ###########
            path="./loss/loss_log_"+config.dataset+".txt"
            with open(path, "a") as f:
                ...
                if config.class_conditional:
                    f.write(f"Loss_target_domain = {loss_target_domain.detach().item():.6f}, ")
                f.write(f"LR = {lr_scheduler.get_last_lr()[0]:.8f}\n")
            #############
        ...
        flag=1
        if (epoch + 1) % config.save_image_epochs == 0 or epoch == config.num_epochs - 1 or epoch == 19 or epoch==early_stop-1:
            # if config.segmentation_guided:
            #     try:
            #         seg_batch = next(eval_iter)
            #     except (NameError, StopIteration):  # 第一次或已经迭代完
            #         eval_iter = iter(original_eval_dataloader)
            #         seg_batch = next(eval_iter)
            ...
            if config.segmentation_guided:
                try:
                    ...
                    temp_eval_dataloader=original_eval_dataloader
                    eval_dataloader = iter(temp_eval_dataloader)
                    #seg_batch = next(eval_dataloader)
                    flag=0
                if flag==1:
                    evaluate(config, epoch, pipeline, seg_batch)
        ...
        if ((epoch + 1) % config.save_model_epochs == 0 or epoch == config.num_epochs - 1 or epoch == 0 or epoch==early_stop-1) and flag==1:
        ...
        # 写入txt文件
        path_2='./run_time/epoch_train_time/epoch'+config.dataset+'.txt'
        with open(path_2, 'a') as f:
            f.write(log)
        
        if epoch==early_stop-1:
            break
