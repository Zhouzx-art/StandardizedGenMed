...
def main(
    ...
    if config.segmentation_guided:
        ...
        if img_dir is not None:
            def transform(examples):
                if not load_images_as_np_arrays:
                    images = [preprocess(image.convert(PIL_image_type)) for image in examples["image"]]
                    #print("Image stats:", images[0].shape, images[0].min().item(), images[0].max().item())
    ...
    if (mode == "train" and resume_epoch is not None) or "eval" in mode:
        ...
        if resume_epoch is not None:
            if resume_epoch > 1 or resume_epoch==0:
                save_path = os.path.join(config.output_dir, f"epoch_{resume_epoch-1}")

        if "eval" in mode:
            save_path = os.path.join('./', save_path)
            save_path=save_path+"/epoch_"+str(epoch_num)
            model = model.from_pretrained(os.path.join(save_path, 'unet'), use_safetensors=True)
        else:
            model = model.from_pretrained(os.path.join(save_path, 'unet'), use_safetensors=True)
    ...
    if mode == "train":
        ...
        path_temp1='./run_time/total_train_time/total_'+dataset+'.txt'
        with open(path_temp1, 'a') as f:
            f.write(log)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--eval_name', type=str, default="train")
    parser.add_argument('--seed', type=int, default=2)
    parser.add_argument('--lr_warmup_steps', type=int, default=500)
    parser.add_argument('--learning_rate', type=float, default=1e-4)
    parser.add_argument('--gradient_accumulation_steps', type=int, default=1)
    parser.add_argument('--save_path_num', type=int, default=1)
