...
from generate import ge

def build_everything(args: arg_util.Args):
    # resume
    auto_resume_info, start_ep, start_it, trainer_state, args_state = auto_resume(args, 'all_states_last.pth')
    ...
    if not args.local_debug:
        ...
        num_classes, dataset_train, dataset_val = build_dataset(
            args.data_path, args.condition_path, args.pn, final_reso=args.data_load_reso, hflip=args.hflip, mid_reso=1.0,
        )
    ...
    vae_ckpt = args.ge_vae_ckpt
    pretrained_var_ckpt = args.ge_var_ckpt
...
def main_training():
    ...
    for ep in range(start_ep, args.ep):
        ...
        #is_val_and_also_saving = (ep + 1) % 50 == 0 or (ep + 1) == args.ep  or ep==9
        is_val_and_also_saving=(ep+1)%5==0 or ep==1 or (ep+1)==args.ep
        if is_val_and_also_saving:
            ...
            if dist.is_local_master():
                save_name='ar-ckpt-'+str(ep+1)+'.pth'
                local_save_ckpt = os.path.join(args.local_out_dir_path, save_name)
                ...
                if (ep + 1) % args.save_iter == 0 or (ep + 1) == args.ep:
                    #shutil.copy(local_out_all_states_last, local_out_all_states_best)
                    shutil.copy(local_out_car_ckpt_last, local_save_ckpt)
        ...
        print(f'\n[ep{ep}]  (training )  Lm: {L_mean:.3f} ({best_L_mean:.3f}), Lt: {L_tail:.3f} ({best_L_tail:.3f}),'
              ...
              flush=True)
        path="./loss/"+args.loss_name+".txt"
        with open(path, 'a') as f:  # 使用 'a' 模式是追加写入
            print(f'\n[ep{ep}]  (training )  Lm: {L_mean:.3f} ({best_L_mean:.3f}), '
                f'Lt: {L_tail:.3f} ({best_L_tail:.3f}), '
                f'Acc m&t: {best_acc_mean:.2f} {best_acc_tail:.2f},  '
                f'Remain: {remain_time},  Finish: {finish_time}\n',
                file=f, flush=True)
        path_2="./epoch_time/"+args.loss_name+".txt"
        elapsed_epoch=time.time()-start_time_1
        log = f"epoch:{ep:.0f}_spend: {elapsed_epoch:.2f} 秒\n"
        with open(path_2, 'a') as f: 
            f.write(log)
        if (ep+1)%5==0:
            ge(ep,args.ge_vae_ckpt,args.ge_var_ckpt,args.ge_car_ckpt,args.condition_path,args.data_path,args.ge_save_dir,args.Pic_id)
       
        #if (ep+1)%50==0  or ep==9:
            #ge(ep,args.ge_vae_ckpt,args.ge_var_ckpt,args.ge_car_ckpt,args.condition_path,args.data_path,args.ge_save_dir,args.Pic_id)
