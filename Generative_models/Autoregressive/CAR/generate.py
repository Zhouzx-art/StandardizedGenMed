...
#def ge(num,local_out_dir_path,loss_name):
def ge(num,ge_vae_ckpt,ge_var_ckpt,ge_car_ckpt,ge_mask_dir,ge_imgs_dir,ge_save_dir,Pic_id):
    setattr(torch.nn.Linear, 'reset_parameters', lambda self: None)  # disable default parameter init for faster speed
    setattr(torch.nn.LayerNorm, 'reset_parameters', lambda self: None)  # disable default parameter init for faster speed
    vae_ckpt=ge_vae_ckpt
    var_ckpt=ge_var_ckpt
    car_ckpt=ge_car_ckpt
    os.makedirs(ge_save_dir, exist_ok=True)
    save_path=ge_save_dir+"/result_"+str(num)+".png"
    mask_path=ge_mask_dir+"/"+Pic_id
    MODEL_DEPTH = 16
    ...
    var_weights = torch.load(var_ckpt, map_location='cpu')
    car_weights = torch.load(car_ckpt, map_location='cpu')
    all_weights = {}
    ...
    mask = PImage.open(mask_path).convert('L')  # 单通道灰度图
