...
def main(args):
    ...
    #patch_nums = (16,16,16,16,16,16,16,16,16,16)
    ...
    # 加载灰度 mask 图像，值为 0 / 127 / 255，转换为 RGB 格式
    mask = PImage.open(args.mask_path).convert('L')  # 单通道灰度图
    mask_np = np.array(mask)
    print(mask_np.shape)

    # 转为 RGB（3 通道），shape: [H, W, 3]
    mask_np_rgb = np.stack([mask_np] * 3, axis=-1)
    control = PImage.fromarray(mask_np_rgb.astype(np.uint8))  # 仍然是 PIL.Image 类型，保持后续一致性


    control_images, control_tensors = get_control_for_each_scale(control, patch_nums)
    class_labels = [1] * 8  # 固定为 0 类
    B = len(class_labels)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--vae_ckpt', type=str, default='/home/zhouzx/CAR_VAE/checkpoints/vqvae_origa_epoch300.pth')
    parser.add_argument('--var_ckpt', type=str, default='/home/zhouzx/CAR/pre_ckpt/var_ckpt/origa_300_var.pth')
    parser.add_argument('--car_ckpt', type=str, default='/home/zhouzx/CAR/local_output_hed_new_lr0.0001/car_ckpt_last.pth')
    parser.add_argument('--save_path', type=str, default='/home/zhouzx/CAR/results/result0.png')
    parser.add_argument('--mask_path', type=str, default='/home/zhouzx/CAR/sample_images/imgs_val/001.png')
