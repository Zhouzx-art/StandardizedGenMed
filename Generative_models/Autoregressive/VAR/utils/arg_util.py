...
def safe_git_command(cmd, fallback='[unknown]'):
    try:
        return subprocess.check_output(cmd, shell=True).decode('utf-8').strip() or fallback
...
class Args(Tap):
    ...
    exp_name: str = 'text'
    #####################################
    ge_vae_ckpt:str =None
    ge_var_ckpt:str=None
    ge_save_path:str=None
    dataset_name:str=""
    save_iter:int=1000
    #####################################
    ...
    branch: str = safe_git_command('git symbolic-ref --short HEAD 2>/dev/null || git rev-parse HEAD')
    commit_id: str = safe_git_command('git rev-parse HEAD')
    commit_msg: str = (safe_git_command('git log -1').splitlines() or ['[unknown]'])[-1].strip()
