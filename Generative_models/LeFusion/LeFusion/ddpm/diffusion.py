...
class GaussianDiffusion_Nolatent(nn.Module):
    ...
    def p_sample_loop_repaint(self,
        ...
        if self.data_type == 'fundus':
            gt_keep_mask = model_kwargs.get('gt_keep_mask')
            mask = (gt_keep_mask > 0.1).float()  
            mask = mask.eq(0) 
            mask1 = (gt_keep_mask > 0.7).float() 
            mask2 = ((gt_keep_mask < 0.7) & (gt_keep_mask > 0.1)).float() 
            final1, final2 = torch.split(final, final.size(1) // 2, dim=1)
            final = final1 * mask1 + final2 * mask2 + final1 * mask
        return final 
    ...
    def p_sample_repaint(
        ...
        elif self.data_type == 'isic' or self.data_type == 'polyp' or self.data_type == 'prostate' or self.data_type == 'nuclei' or self.data_type == 'fets' or self.data_type == 'chaos':
            mask = (gt_keep_mask >0.3 ).float()
        elif self.data_type == 'fundus':
            mask = (gt_keep_mask >0.1).float()
    ...
    def p_losses(self, x_start, t, mask, cond=None, noise=None, **kwargs):
        ...
        if self.data_type == 'lidc' :
            ...
            mask1 = (mask >0.3).float()
            noise = noise * mask1
            x_recon = x_recon * mask1
            if self.loss_type == 'l1':
                loss = F.l1_loss(noise, x_recon)
            elif self.loss_type == 'l2':
                loss = F.mse_loss(noise, x_recon)
            ...
            mask1 = (mask >0.7).float()
            mask2 = ((mask >0.1) & (mask <0.7)).float()
...
class Trainer(object):
    ...
    def train(
        ...
        start_time = time.time()  # Start the timer at the beginning of training
        while self.step < self.train_num_steps:
            ...
            if self.step % 1000 == 0:
                ...
                path=self.data_type+'_time_log.txt'
                with open(path, 'a') as f:
                    f.write(f'Step {self.step}: {elapsed_time:.2f} seconds\n')
                print(f"Time at step {self.step}: {elapsed_time:.2f} seconds")
