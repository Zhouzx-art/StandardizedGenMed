...
class ISICDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # 计算图像直方图
        #mask_temp = mask.expand_as(img)
        #hist = torch.histc(img[mask_temp > 0], bins=16 min=-1, max=1) / mask_temp.sum()
        hists = []
        for channel in range(3):  # RGB三个通道
            channel_img = img_tensor[channel]  # 单通道图像 [H, W]
            # 只计算病变区域的像素
            channel_pixels = channel_img[mask_tensor[0] > 0]  # mask_tensor形状为[1, H, W]
        ...
        # 合并三个通道的直方图（16*3=48维特征）
       # hist = (hists[0] + hists[1] + hists[2]) / 3.0 
        # 拼接三个通道的直方图
        hist = torch.cat(hists, dim=0)  # [48]
        # 如果直方图出现问题（例如全零或NaN），则输出信息
